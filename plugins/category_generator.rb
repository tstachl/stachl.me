...
module Jekyll
  ...
  module Filters
    ...
    # Returns string
    #
    def category_links(categories, current = nil)
      dir = @context.registers[:site].config['category_dir']
      if current
        category = current['category']
        category ||= if defined? current['categories'] and current['categories'] then current['categories'].first else nil end      
        categories = categories.keys if categories.class == Hash
        categories = categories.sort!.map do |item|
          "<li class='#{if category == item then 'active' end}'><a class='category' href='/#{dir}/#{item.gsub(/_|\P{Word}/, '-').gsub(/-{2,}/, '-').downcase}/'>#{item}</a></li>"
        end
      else
        categories = categories.sort!.map do |item|
